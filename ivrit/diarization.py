...
class PyannoteDiarizationEngine(BaseDiarizationEngine):
    ...
    def diarize(
        ...
        **kwargs
        ...
        """
        diarization_df = self.run_pipeline(
            audio,
            device=device,
    ...
    def run_pipeline(
        ...
        **kwargs,
        ...
        """
        Run the PyAnnote diarization pipeline on raw audio and return the
        resulting speaker-turns dataframe, WITHOUT assigning speakers to any
        transcription segments.

        This step has no dependency on transcription output, so it can be
        run concurrently (e.g. in a separate thread) with a transcription
        pass over the same audio. Use `assign_speakers` afterwards to merge
        the returned dataframe with transcription segments.
        ...
        Args:
            ...
            use_auth_token: Authentication token for model download.
            verbose: Whether to enable verbose logging.
            on_progress: Optional progress callback. See `diarize`.

        Returns:
            DataFrame with columns ['segment', 'label', 'speaker', 'start', 'end']
            describing the detected speaker turns.
        """
        import numpy as np
        import pandas as pd
        import torch
        ...
        if verbose:
            logger.info("Diarization pipeline completed successfully")
    ...
    def assign_speakers(
        ...
        """
        Public wrapper around `_assign_speakers`, for callers that ran
        `run_pipeline` separately (e.g. concurrently with transcription) and
        now need to merge the result into transcription segments.
        """
        return self._assign_speakers(diarization_df, transcription_segments, fill_nearest=fill_nearest)
